...
interface EditPemeriksaanProps {
  params: { patientId: string }
}

export default function EditPemeriksaanPage({ params }: EditPemeriksaanProps) {
  ...
  // Load existing examination data from localStorage or use mock data
  ...
  useEffect(() => {
    // Try to load existing examination data from localStorage
    const examDataKey = `exam-${selectedDate}-${params.patientId}`
    const savedData = localStorage.getItem(examDataKey)

    if (savedData) {
      setExaminationData(JSON.parse(savedData))
    } else {
      // Use mock existing data for demonstration if no saved data
      const mockExistingData = {
        ...
        bloodSugar: "95",
        uricAcid: "5.2",
        notes: "Tekanan darah sedikit tinggi, disarankan untuk mengurangi konsumsi garam.",
      }
      setExaminationData(mockExistingData)
    }
  }, [selectedDate, params.patientId])

  const handleInputChange = (field: string, value: string) => {
    setExaminationData((prev) => ({ ...prev, [field]: value }))
  }

  // Update the handleSubmit function to maintain consistency
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Save updated examination data to localStorage
    const examDataKey = `exam-${selectedDate}-${params.patientId}`
    localStorage.setItem(examDataKey, JSON.stringify(examinationData))

    // Also update the master key
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-gray-600">Edit data hasil pemeriksaan kesehatan</p>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button type="submit" className="w-full" disabled={isLoading}>
                    ...
                    {isLoading ? "Menyimpan..." : "💾 Update Data Pemeriksaan"}
                  </Button>
                </form>
              </CardContent>
            </Card>
